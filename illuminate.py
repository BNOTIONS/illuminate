...
tileWidth = font.getsize('No Predictions')[0]  # Label when no times are available
...
class tile:
    def __init__(self, x, y, number, label):
        self.x = x
        self.y = y
        self.number = number
        self.label = label
...
if tileWidth >= width: tilesAcross = 2
    ...
    tilesAcross = int(math.ceil(width / tileWidth)) + 1

tileList.append(tile(1 * tileWidth + 1 * tileWidth / 2, 1 * 17, "23", " Users on Slack"))
tileList.append(tile(1 * tileWidth + 1 * tileWidth / 2, 2 * 17, "13", " PRS merged"))
